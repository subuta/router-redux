...
const routerCreator = (store) => {
  ...
  requestAnimationFrame(() => {
    ...
    const route = findRouteKeyByPath(currentPath);
    if (routes[route] && routes[route].onEnter) {
      // if handler accepts callback.
      if (routes[route].onEnter.length == 2) {
        routes[route].onEnter({dispatch, state: getState()}, (data = true) => {
          dispatch(initialRouteResolved());
          routeErrorHandler(data);
        });
      } else {
        routes[route].onEnter({dispatch, state: getState()});
      }
    }
  });
