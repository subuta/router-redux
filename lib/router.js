...
export const createRouteErrorHandler = (store) => (data) => {
...
export const createRouterAction = (store) => {
  const { dispatch, getState } = store;
  const routeErrorHandler = createRouteErrorHandler(store);
  // routerAction
    ...
    if (nextRoute && nextRoute.onEnter) {
      ...
      if (nextRoute.onEnter.length == 2) {
        nextRoute.onEnter({ dispatch, state: getState() }, (data = true) => {
          ...
          dispatch(actions.locationChange(stringifyLocation(next), action));
        });
        return;
      } else {
