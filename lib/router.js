...
export const createRouterAction = (store) => {
  ...
  // routerAction
  return (action, preventNaviation = false) => function routerAction () {
    ...
    if (nextRoute && nextRoute.onEnter) {
      ...
      if (nextRoute.onEnter.length == 2) {
        nextRoute.onEnter({ dispatch, state: getState() }, (data = true) => {
          ...
          dispatch(actions.locationChange(stringifyLocation(next), preventNaviation));
        });
        return;
      } else {
...
export default (store, option = {}) => {
  ...
  const pushWithoutNavigation = routerAction('push', false);

  // handle store.update(for redux time travel)
  let last = history.getLocation();
  let ignoreActions = false;
  const unsubscribe = subscribe(() => {
    const current = getCurrent(getState());

    if (ignoreActions || !current) return;
    if (stringifyLocation(last) === stringifyLocation(current)) return;
  ...
  const unlisten = history.listen(() => {
    ...
    ignoreActions = true;
    last = current;
    pushWithoutNavigation(stringifyLocation(current), false);
    ignoreActions = false;
  });
