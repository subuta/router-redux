...
// TODO: pass matched parameter to state.
...
const callHistoryByAction = (history, action) => {
  ...
  action.payload = transformToPath(location);
};
...
export default (store) => (next) => (action) => {
  ...
  if (nextRoute && nextRoute.onEnter) {
    ...
    if (nextRoute.onEnter.length == 2) {
      nextRoute.onEnter({dispatch, state}, (data = true) => {
      ...
      nextRoute.onEnter({dispatch, state});
    }
  }
