...
const callHistoryAndUpdateAction = (history, action) => {
  if (action.type === PUSH) {
    history.push ? history.push(action.payload) : history.pushState(null, null, action.payload);
  } else if (action.type === REPLACE) {
    history.replace ? history.replace(action.payload) : history.replaceState(null, null, action.payload);
...
export const routerMiddlewareCreator = (option = {}) => (store) => (next) => (action) => {
  // use current state
  const {dispatch, getState} = store;
  ...
  const history = option.history || window.history;
